...
bool Lambertian::Scatter(const ray_t& ray, const raycastHit_t& raycastHit, color3_t& color, ray_t& scattered) const
{
    ...
    glm::vec3 direction = glm::normalize(target - raycastHit.point); //<create a direction from raycast hit point to the target and normalize it>;
...
{
	glm::vec3 reflected = reflect(glm::normalize(ray.direction), glm::normalize(raycastHit.normal)); // <get reflected ray from normalized ray direction and raycast hit normal>;
	...
	return glm::dot(glm::normalize(scattered.direction), glm::normalize(raycastHit.normal)) > 0;
}